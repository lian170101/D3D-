...
D3DXVECTOR3 cylinderPos(0.0f,0.0f,0.0f);

D3DXMATRIX Ts,Zoom,Ry,Rx;
float y = 0.0f;
float rx = 0.0f;
...
bool Setup()
{
	...
	D3DXCreateCylinder(Device,1.0f,1.0f,3.0f,20,20,&cylinder,0);
	...
	D3DXVECTOR3 position(1.0f, 0.0f, -5.0f);
...
bool Display(float timeDelta)
{
	if( Device ) // Only use Device methods if we have a valid device.
	{
		...
		/*所有的按键提示，不会变*/
		Font2->DrawText(NULL,
			"\n\n\n1、'L'变长，'N'变短，'F'变胖，'T'变瘦\n2、'pageup'、'pagedown'、'home'、'end'，柱体能够上下左右移动\n3、'C'键柱体能够旋转\n4、'R'、'G'、'B'，可以有不同颜色的光照到柱体上；\n5、'P'、'D'、'S'，分别选择点光源，方向灯光源和聚光灯光源。",
			-1,
			&rect,
			DT_TOP|DT_LEFT,
			0xff000000);
		...
		D3DXMATRIX Rx2;
		D3DXMatrixTranslation(&Ts,cylinderPos.x,cylinderPos.y,cylinderPos.z);
		D3DXMatrixScaling(&Zoom,zmx,zmx,zmz);
		D3DXMatrixRotationY(&Ry,y);
		D3DXMatrixRotationY(&Rx,rx);
		D3DXMatrixRotationY(&Rx2,3.14f/1.57f);
		//y += timeDelta;
		D3DXMATRIX p = Ts * Zoom * Ry * Rx *Rx2;
